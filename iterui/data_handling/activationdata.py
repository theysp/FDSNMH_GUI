...
def eval_str_number(num_str):
    new_num_str = num_str
    pos_minus = num_str.find('-')
    if num_str.find('-') > 0:
        ch_before_minus = num_str[pos_minus-1]
        if ch_before_minus != 'e' or ch_before_minus != 'E' :
            new_num_str = num_str[:pos_minus] + 'E' + num_str[pos_minus:]
    try:
...
class OneSpectrumOneStepActivationData:
    ...
    def load_from_raw_lines(self, lines, startidx, endidx):
     ...
     # total info
        # 'total_activity(Bq)'
        self.parameters['total_activity(Bq)'] = eval_str_number(lines[nuclide_data_end_idx + 6][40:51])
        # 'total_activity_no_tritium(Bq)'
        self.parameters['total_activity_no_tritium(Bq)'] = eval_str_number(lines[nuclide_data_end_idx + 7][40:51])
        # 'alpha_heat(kW)'
        self.parameters['alpha_heat(kW)'] = eval_str_number(lines[nuclide_data_end_idx + 8][40:51])
        # 'beta_heat(kW)'
        self.parameters['beta_heat(kW)'] = eval_str_number(lines[nuclide_data_end_idx + 9][40:51])
        # 'gamma_heat(kW)'
        self.parameters['gamma_heat(kW)'] = eval_str_number(lines[nuclide_data_end_idx + 10][40:51])
        # 'total_heat(kW)'
        self.parameters['total_heat(kW)'] = eval_str_number(lines[nuclide_data_end_idx + 10][90:101])
        # 'total_heat_ex_tritium(kW)'
        self.parameters['total_heat_ex_tritium(kW)'] = eval_str_number(lines[nuclide_data_end_idx + 11][90:101])
        # 'origin_mass(kg)'
        self.parameters['origin_mass(kg)'] = eval_str_number(lines[nuclide_data_end_idx + 11][40:51])
        # 'cur_mass(kg)'
        self.parameters['cur_mass(kg)'] = eval_str_number(lines[nuclide_data_end_idx + 12][40:51])
        # 'neutron_flux(n/cm**2/s)'
        self.parameters['neutron_flux(n/cm**2/s)'] = eval_str_number(lines[nuclide_data_end_idx + 13][40:51])
        # 'number_fission'
        self.parameters['number_fission'] = eval_str_number(lines[nuclide_data_end_idx + 14][40:51])
        # 'ingestion_dose(Sv/kg)'
        self.parameters['ingestion_dose(Sv/kg)'] = eval_str_number(lines[nuclide_data_end_idx + 15][40:51])
        # 'inhalation_dose(Sv/kg)'
        self.parameters['inhalation_dose(Sv/kg)'] = eval_str_number(lines[nuclide_data_end_idx + 16][40:51])
        # 'ingestion_dose_ex_tritium(Sv/kg)'
        self.parameters['ingestion_dose_ex_tritium(Sv/kg)'] = eval_str_number(lines[nuclide_data_end_idx + 17][40:51])
        # 'inhalation_dose_ex_tritium(Sv/kg)'
        self.parameters['inhalation_dose_ex_tritium(Sv/kg)'] = eval_str_number(lines[nuclide_data_end_idx + 18][40:51])
        # 'gase_rate(appm/sec)'
        self.parameters['inhalation_dose_ex_tritium(Sv/kg)'] = eval_str_number(lines[nuclide_data_end_idx + 19][27:38])
        # spectra
        spectra_beg_idx = -1
        spectra_end_idx = -1
        ...
        for i in range(spectra_beg_idx, spectra_end_idx):
            (erg, gamma_power, gammas) = (eval_str_number(lines[i][65:69]),
                                          eval_str_number(lines[i][77:88]),
                                          eval_str_number(lines[i][130:141]))
            self.gamma_erg_bin.append(erg)
...
if __name__ == '__main__':
    ...
    data_test.read_raw_file('C:/Users/ysp/Desktop/QT_practice/ITER DATA/Flux2/B/B.out')